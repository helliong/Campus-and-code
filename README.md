...
Товары и университеты для публичного каталога загружаются из PostgreSQL через Prisma. Базовый справочник университетов добавляется безопасной миграцией `20260718190000_add_university_reference_data`. `prisma/seed.ts` содержит демонстрационные товары и пользователей и предназначен только для локальной разработки.
...
4. При необходимости заполнить локальную базу демонстрационными данными:
   ...
   Не запускайте seed на production: миграции уже создают справочник университетов, а seed дополнительно добавляет тестовые товары и пользователей.
